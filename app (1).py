...
class ManagedProcess:
    """Expose a Popen-like process with a TTY-backed stdin on POSIX hosts."""
        ...
        for uname, data in users.items():
            ...
            for s in servers:
                if isinstance(s, dict) and s.get('server_id') == server_id:
                    server = s
                    break
        if not server or server.get('status') != 'running' or not server.get('rate_limit_enabled', True):
            return False, 0
        pid = server.get('pid')
...
def run_bot(server_id, main_file='main.py', requirements_file='requirements.txt'):
    ...
    try:
        ...
        pty_master = None
        if os.name == 'posix':
            # Many interactive Python programs require a TTY-like stdin. A pipe
            # can make input() wait forever in hosted/background execution.
            import pty
            ...
            os.close(pty_slave)
            stdin_handle = os.fdopen(os.dup(pty_master), 'w', encoding='utf-8', buffering=1)
            proc = ManagedProcess(child, stdin_handle)
        else:
            child = subprocess.Popen(
                [python_exe, main_path_abs],
...
def api_create_server():
    ...
    rate_limit_enabled = request.args.get('rate_limit_enabled', 'true').strip().lower() not in ('0', 'false', 'off', 'no', 'unlimited')
    ...
    if rate_limit_enabled and (cpu_limit < 10 or cpu_limit > 100):
...
def login():
    if request.method == 'POST':
        ...
        users = load_users()
        if username == 'shayan' and password == str(users.get('shayan', {}).get('password', '')):
            session['user'] = 'shayan'
            session['role'] = 'admin'
            return redirect(url_for('admin_dashboard'))
        user_data = users.get(username)
        if user_data and password == str(user_data.get('password', '')).lower():
            servers = user_data.get('servers', [])
            if not isinstance(servers, list) or not servers:
                return render_template('login.html', error="No server is assigned to this account!")
            selected_server = None
            for server in servers:
                if not isinstance(server, dict):
                    continue
                valid, result = check_server_valid(server.get('server_id', ''))
                if valid:
                    selected_server = result
                    break
            if not selected_server:
                return render_template('login.html', error="All servers for this account are expired or deleted!")
            session['user'] = username
            session['role'] = 'user'
            session['current_server_id'] = selected_server['server_id']
            return redirect(url_for('server_home', server_id=selected_server['server_id']))
...
def server_login(server_id):
    ...
    if request.method == 'POST':
        ...
        for uname, data in users.items():
            ...
            for s in servers:
                if isinstance(s, dict) and s.get('server_id') == server_id:
                    if username == uname and password == str(data.get('password', '')):
                        session['user'] = uname
                        session['role'] = 'user'
...
def create_server():
    ...
    new_server = {
        ...
        'cpu_limit': cpu_limit, 'rate_limit_enabled': rate_limit_enabled, 'rate_limit_exceeded': False, 'stopped_by_user': False
    }
    ...
    return jsonify({
        ...
        'hostname': request.host_url.rstrip('/') + '/login',
        'server_id': server_id
    })
...
def set_rate_limit(server_id):
    ...
    cpu_limit = int(payload.get('cpu_limit', 80))
    if rate_limit_enabled and (cpu_limit < 10 or cpu_limit > 100):
        ...
        for s in servers:
            if isinstance(s, dict) and s.get('server_id') == server_id:
                ...
                if not rate_limit_enabled:
                    s['rate_limit_exceeded'] = False
                save_users(users)
                return jsonify({'success': True, 'cpu_limit': cpu_limit, 'rate_limit_enabled': rate_limit_enabled})
...
def api_stats(server_id):
    server, _ = get_server_by_id(server_id)
    if not server:
        return jsonify({'cpu': '0%', 'ram': '0 MB', 'uptime': '0h', 'status': 'unknown', 'cpu_limit': 80, 'rate_limit_enabled': False, 'net_in': '0 KB', 'net_out': '0 KB'})
    ...
    return jsonify({'cpu': cpu, 'ram': ram, 'uptime': uptime, 'net_in': net_in, 'net_out': net_out, 'cpu_limit': server.get('cpu_limit', 80), 'rate_limit_enabled': server.get('rate_limit_enabled', True), 'status': server.get('status', 'stopped')})
...
def api_download_file(server_id):
    if session.get('role') != 'user' or session.get('current_server_id') != server_id:
        return jsonify({'error': 'Please login to this server first'}), 403
    filepath = safe_server_path(server_id, request.args.get('filename', ''))
...
def api_upload(server_id):
    ...
    filepath = safe_server_path(server_id, os.path.join(folder, file.filename))
    if not filepath:
...
def api_create_folder(server_id):
    data = request.get_json()
    folder = safe_server_path(server_id, data.get('foldername', ''))
    if not folder:
        return jsonify({'error': 'Invalid folder'}), 400
...
def api_rename(server_id):
    d = request.get_json()
    server_dir = get_server_dir(server_id)
    old_path = safe_server_path(server_id, d.get('old_name', ''))
    new_path = safe_server_path(server_id, d.get('new_name', ''))
